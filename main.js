...
app.post("/signup", (req,res)=>{
    ...
    conn.query("SELECT email FROM  users WHERE email = ?", [req.body.email], (selectErr, data)=>{
        if (data.length>0) {
            res.render("signup.ejs" , { emailError:"Email already exists"})
        }else{
            if(req.body.confirmPassword == req.body.password){
                const hashedPassword = bcrypt.hashSync(req.body.password, saltRounds)
                conn.query("INSERT INTO users(name,email,password,yob,phone) VALUES(?,?,?,?,?)", [req.body.fullname,req.body.email, hashedPassword,req.body.yob,req.body.phone ],(err)=>{
                    if(err){
                ...
                res.render("signup.ejs" , { passwordError:"Password and confirm password do not match"})
            }
...
app.post("/login", (req,res)=>{
    ...
    conn.query("SELECT * FROM  users WHERE email = ?", [req.body.email], (selectErr, data)=>{
        if (data.length>0) {
            // the person is registerd--proceed to check password
            let isPasswordCorrect = bcrypt.compareSync(req.body.password, data[0].password) // true or false
